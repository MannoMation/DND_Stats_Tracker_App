...
def remove_selected():
    ...
    selected_left = left_box.curselection()
    selected_right = right_box.curselection()

    if selected_left:
        uuid = left_box.get(selected_left[0], selected_left[0])[0].split("UUID:")[1]
        for c in characters:
            if c["UUID"] == uuid:
                c["Archive"] = True
                c["Initiative"] = None

        left_box.delete(selected_left)
    elif selected_right:
        uuid = right_box.get(selected_right[0], selected_right[0])[0].split("UUID:")[1]
        for c in characters:
            if c["UUID"] == uuid:
                c["Archive"] = True
                c["Initiative"] = None

        right_box.delete(selected_right)
    else:
        #messagebox.showinfo("Error", "No option selected.")
        pass


def ConfigureCharacters(root, chars):
    global cc_window
    global left_box
    global right_box
    global characters
    ...
    cc_window.title("Configure Characters")
    ...
    right_label = Label(right_frame, text="Selected Characters")
    right_label.pack()
    ...
    btn_remove = Button(button_frame, text="Archive", command=remove_selected)
